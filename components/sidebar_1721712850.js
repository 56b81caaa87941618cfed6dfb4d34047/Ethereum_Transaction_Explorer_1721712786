Vue.component('sidebar_1721712850', { template: `<nav class="fixed top-0 left-0 w-fit h-full bg-gray-100 space-y-8 sm:w-80 pt-10 text-black z-20">
    ...
    <div class="flex-1 flex flex-col h-full overflow-auto">
        ...
        <template v-for="(item, idx) in navigation" :key="idx">
          <li>
            <a :href="item.href" class="flex items-center gap-x-2 text-white-600 p-5 rounded-lg hover:bg-black hover:text-white active:bg-gray-100 w-fit duration-150">
              <div class="text-white-500" x-html="item.icon"></div>
              <span> { item.name } </span>
            </a>
          </li>
        </template>
      ...
      <div>
        ...
        <div class="py-4 px-4 w-fit border-t border-black">
          <div class="flex items-centergap-x-4">
            <div>
              <span class="block text-white-700 text-sm font-semibold">Track New Ethereum Transactions</span>
              <a href="javascript:void(0)" class="block mt-px text-white-600 text-xs">Get real-time alerts for the latest transactions on the Ethereum blockchain</a>
            </div>
          </div>
        </div>
      </div>
